...
class PGP {
  ...
  public static PgpPublicKey ReadPublicKey(Stream inputStream) {
    ...
    // encryption, in the real world you would probably want to be a bit smarter
    // about this.
    //
    //
   ...
   */
  public static byte[] Encrypt(byte[] inputData, PgpPublicKey passPhrase,
                               bool withIntegrityCheck, bool armor) {
    byte[] processedData = Compress(inputData, PgpLiteralData.Console,
  ...
  private static byte[] Compress(byte[] clearData, string fileName,
    ...
    Stream pOut = lData.Open(cos, // the compressed output stream
